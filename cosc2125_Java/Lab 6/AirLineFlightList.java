...
public class AirLineFlightList implements SortedList
{
	...
	System.out.println("Enter file to read flight data from: (flight.txt) ");
 ...
 */
    ...
    /**receives flight object from menu option to add reservation, or receives flight
     object from add flight to flightList */
	public void addReservation(Object object) 
	{
		...
		AirLineFlight flight = (AirLineFlight) object;
		int loc = binSearch(flight);
		if (loc != -1)
		    {
			list_[loc].addReservation();
		    }
		return;	
		}//end of addReservation

	public void removeReservation(AirLineFlight flight) 
	{
		int loc = binSearch(flight);
		if (loc != -1)
	    {
		list_[loc].removeReservation();
	    }
		return;
	}

	public void addPassenger(AirLineFlight flight) 
	{
		int loc = binSearch(flight);
		if (loc != -1)
	    {
		list_[loc].addPassenger();
	    }
		return;
	}
